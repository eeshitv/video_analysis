...
load('/Users/eesh/Desktop/video_analysis_data/water_injection/Image5_121813/Measurements/Membranes--basic_2d--Area.mat'); %this loads the y 
area=data;
...
for cell_index=union([1:1:38],[40:1:cell_number]), %this mega for loop calculates the COM for all the cells which are taken from the edge output
   ...
   % SE = strel('octagon',3);
    ...
    cell(cell_index).mean = (2/cell(cell_index).mean(1,1))*cell(cell_index).mean ;
     ...
     y=cell(cell_index).mean(:,:)';
        ...
        %p=polyfit(x,y,10);      %%I DONT THINK YOU WANT THIS 10 here!!! see this line
    
        %f = polyval(p,x);

       %%horizontal line intersection part
       f=cell(cell_index).mean(:,:)';
